...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!therapistId || !startTime || !endTime) {
      throw new Error("Missing required appointment details: 'therapistId', 'startTime', and 'endTime' are required.")
    }
    ...
    if (isNaN(start.getTime()) || isNaN(end.getTime())) {
      throw new Error("Invalid date format provided. Please use ISO 8601 format.")
    }

    // 2b. Validate Future Dates
    if (start < now) {
      throw new Error("Cannot book appointments in the past. Please choose a future time.")
    }
    ...
    if (startHour < 9 || startHour >= 18) {
      throw new Error(`Appointments can only be booked between 9:00 AM and 6:00 PM (${timeZoneToUse}).`)
    }
    ...
    if (adminTherapist?.google_refresh_token) {
      console.log(`✓ Admin calendar credentials found. Routing booking to this calendar.`);
      ...
      try {
        ...
        const eventBody = {
          ...
          description: `Internal Therapist ID: ${therapistId}\nInquiry ID: ${inquiryId}\n\n(Routed to Admin Calendar)`,
          start: { 
            dateTime: startTime,
            timeZone: timeZoneToUse
          },
        ...
        if (!calendarResponse.ok) {
            ...
            throw new Error(`Google Calendar sync failed: ${calendarResponse.statusText}`)
        }

        const eventData = await calendarResponse.json()
        if (eventData.id) {
           googleCalendarEventId = eventData.id;
           console.log(`✓ Google Calendar sync successful (Event ID: ${googleCalendarEventId})`);
        }
      ...
      console.warn("No connected Admin Calendar found (no therapist has a refresh token). Skipping Calendar Sync.");
